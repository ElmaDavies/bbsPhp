...
if($_GET['action'] == 'delall' && isset($_POST['ids']))
{
	...
	if(!!$_rows2 = _fetch_array("SELECT tg_uniqid FROM ta_user WHERE tg_username='{$_COOKIE['username']}'"))
	{
		...
		mysql_query("DELETE FROM tg_message WHERE tg_id  IN({$_clean['ids']}) ");
		if(_affected_rows())
		{
			mysql_close();
...
$_nums = mysql_num_rows(mysql_query("SELECT tg_id,tg_to_user FROM tg_message WHERE tg_to_user='{$_COOKIE['username']}'"));
//分页参数
$_pagesize = 12;
...
$sql = "SELECT tg_id,tg_to_user,tg_state,tg_from_user,tg_date,tg_content FROM tg_message WHERE tg_to_user='{$_COOKIE['username']}' ORDER BY tg_date DESC LIMIT $_page_num,$_pagesize";
...
<div id="member">
  ...
  <div id="main">
  <h2><?php echo $_system['webname']?>--短信管理中心</h2>
  <form method="post" action="?action=delall">
  <table cellspacing="1">
  <tr><th>发信人</th><th>短信内容</th><th>发送时间</th><th>状态</th><th>操作</th></tr>
  <?php while(!!$rows=mysql_fetch_array($_result)){
  	$_html = array();
  	$_html['id'] = $rows['tg_id'];
  	$_html['username'] = $rows['tg_from_user'];
  	$_html['date'] = $rows['tg_date'];
  	$_html['content'] = $rows['tg_content'];
  	if(empty($rows['tg_state']))
  	{
  		$_html['state'] = '<img alt="短信状态" title="未读" src="images/unread.gif" />';
  	}else{ 
  		$_html['state'] = '<img alt="短信状态" title="已读" src="images/read.gif" />';
  	}
  
  ?>
  <tr><td><?php echo $_html['username'];?></td><td><a href="member_message_detail.php?id=<?php echo $_html['id']?>"  title=<?php echo $_html['content']?>><?php echo _title($_html['content'],12)?></a></td><td><?php echo $_html['date']?></td><td><?php echo $_html['state']?><td><input name="ids[]" value="<?php echo $_html['id']?>" type="checkbox" /></td></tr>
  <?php 
  }?>
  ...
  <div id="page_text">
    <ul>
	  ...
	  <li>共有<strong><?php echo $_nums;?></strong>条短信| </li>
	  <?php 
	  if($_page == 1)
	  {
	  	echo '<li>首页| </li>';
