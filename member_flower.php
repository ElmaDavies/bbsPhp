...
if($_GET['action'] == 'delall' && isset($_POST['ids']))
{
	...
	if(!!$_rows2 = _fetch_array("SELECT tg_uniqid FROM ta_user WHERE tg_username='{$_COOKIE['username']}'"))
	{
		...
		mysql_query("DELETE FROM tg_flowers WHERE tg_id  IN({$_clean['ids']}) ");
		if(_affected_rows())
		{
			mysql_close();

			echo "<script>alert('删除成功');location.href='member_flower.php';</script>";
...
$_nums = mysql_num_rows(mysql_query("SELECT tg_id,tg_to_user FROM tg_flowers WHERE tg_to_user='{$_COOKIE['username']}' "));
//分页参数
$_pagesize = 12;
...
$sql = "SELECT tg_id,tg_to_user,tg_count,tg_from_user,tg_date,tg_content FROM tg_flowers WHERE tg_to_user='{$_COOKIE['username']}' ORDER BY tg_date DESC LIMIT $_page_num,$_pagesize";
...
<div id="member">
  ...
  <div id="main">
  <h2><?php echo $_system['webname']?>--送花管理中心</h2>
  <form method="post" action="?action=delall">
  <table cellspacing="1">
  <tr><th>送花者</th><th>送花留言</th><th>送花时间</th><th>送花数量</th><th>操作</th></tr>
  ...
  while(!!$rows=mysql_fetch_array($_result)){
  	...
  	$_html['count'] = $rows['tg_count'];
  	$_html['date'] = $rows['tg_date'];
  	$_html['content'] = $rows['tg_content'];
    $_html['counts']+=$_html['count'];
  	
  
  ?>
  <tr><td><?php echo $_html['from_user'];?></td><td  title='<?php echo $_html['content'];?>'><?php echo _title($_html['content'],12);?></td><td><?php echo $_html['date']?></td><td><img src="images/flower.jpg" />X<?php echo $_html['count']?>朵<td><input name="ids[]" value="<?php echo $_html['id']?>" type="checkbox" /></td></tr>
  <?php 
  }?>
  <tr><td colspan=5>共<?php echo $_html[counts];?>朵</td></tr>
